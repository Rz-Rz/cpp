    ...
    Compare compare=Compare())
{
  ...
  {
    typename std::list<RandomAccessIterator>::iterator next_it = it;
    ++next_it; // Equivalent to std::next(it)
    typename std::list<RandomAccessIterator>::iterator tmp = chain.insert(chain.end(), next_it);
    node n = {it, tmp}; // Create a node object
    pend.push_back(n);
  }
  ...
  {
    // Find next index
    uint_least64_t dist = jacobsthal_diff[k];
    if (dist >= pend.size()) break;
    ...
    while (true)
    {
      std::list<RandomAccessIterator>::iterator insertion_point = std::upper_bound(
          chain.begin(), it->next, it->it,
          CompareFunction(compare) // Assuming CompareFunction is a functor or function that encapsulates the comparison logic
          );
      chain.insert(insertion_point, it->it);
